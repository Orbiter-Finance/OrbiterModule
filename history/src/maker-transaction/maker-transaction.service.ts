...
export class MakerTransactionService {
  ...
  async findAll(query: PaginationReqRO): Promise<PaginationResRO<any>> {
    ...
    if (query.startTime) {
      more += `and t2.timestamp >= '${formateTimestamp(+query.startTime)}' `;
    }
    if (query.endTime) {
      more += `and t2.timestamp <= '${formateTimestamp(+query.endTime)}' `;
    }
    if (query.fromChain) {
    ...
    const sql = `
      select m.id, m.transcationId, m.fromChain, m.toChain, m.toAmount, m.replySender, m.replyAccount, m.createdAt, m.updatedAt,
        ...
        t.feeToken as tokenName, t2.status as status, t.status as fromStatus 
          ...
          where m.inId is not null and m.outId is not null ${more} 
          order by t2.timestamp DESC, t.timestamp DESC, m.createdAt DESC
