    ...
    const makerSend = (makerAddress, chainId) => {
        const callback = async () => {
            const sn = async () => {
                ...
                let waringInterval = consulConfig.starknet.waringInterval || 180;
                // Execute several transactions at once
                let execTaskCount = consulConfig.starknet.execTaskCount || 50;
                // Maximum number of transactions to be stacked in the memory pool
                let maxTaskCount: number = consulConfig.starknet.maxTaskCount || 200;
                let expireTime: number = consulConfig.starknet.expireTime || 30 * 60;
                let maxTryCount: number = consulConfig.starknet.maxTryCount || 180;
...
export function watchStarknetAlarm() {
    let waringInterval = consulConfig.starknet.waringInterval || 180;
    if (cron) {
        clearInterval(cron);
    }
