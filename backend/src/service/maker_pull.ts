...
export class ServiceMakerPull {
  ...
  */
  async zksync2(chainInfo) {
    ...
    let data = await this.zksync2GetTxlistByApi(chainInfo, endblock, makerPullLastData.pullCount)
    const promiseMethods: (() => Promise<unknown>)[] = []
  ...
  private async zksync2GetTxlistByApi(chainInfo, endblock, pullCount) {
    ...
    if (endblock) {
      startBlockNumber = Number(endblock) - (pullCount + 1) * 5000 > 0 ? Number(endblock) - (pullCount + 1) * 5000 : 0
    }
    try {
      ...
      if (respData.status == '1' && respData.result) {
        ...
        while (originTxList.length) {
          if (originTxList[0].from.toLowerCase() == this.makerAddress.toLowerCase()) {
            let txIndex = realTxList.findIndex((item) => item.hash == originTxList[0].hash)
            if (txIndex > 0 && realTxList[txIndex].logIndex < originTxList[0].hash) {
            ...
            } else if (txIndex > 0 && realTxList[txIndex].logIndex > originTxList[0].hash) {
              realTxList[txIndex].gasAmount = originTxList[0].value
              realTxList[txIndex].feeTokenAddress = originTxList[0].contractAddress
  ...
  private async zksync2GetTxlistByWeb3(chainInfo, tokenAddress, makerAddress) {
    ...
    return await this.getTxListTenByWeb3(chainInfo, zk2Web3, tokenAddress, tokenContract, makerAddress)
  }

  private async getTxListTenByWeb3(chainInfo, zk2Web3, tokenAddress, tokenContract, makerAddress) {
    let zk2BlockNumberInfo = {}// if active the func please remove it.it is at top of the page
    let txList: any[] = []
    for (let i = 0; i < 10; i++) {
      try {
        const fromTxs: any = await this.getTxListOnceByWeb3(chainInfo.zksync2GasAddress, tokenContract, zk2BlockNumberInfo[tokenAddress].pointBlockNumber, makerAddress, true)
        const toTxs: any = await this.getTxListOnceByWeb3(chainInfo.zksync2GasAddress, tokenContract, zk2BlockNumberInfo[tokenAddress].pointBlockNumber, makerAddress, false)
  ...
  private getTxListOnceByWeb3(zksync2GasAddress, tokenContract, currentBlock, makerAddress, isFrom) {
