  ...
  if (isEthTokenAddress(tokenAddress)) {
    ...
    tokenContract.events
      .Transfer(options, (error, event) => {
        ...
        if (event.returnValues.to.toLowerCase() === makerAddress.toLowerCase()) {
          if (
...
function confirmZKSTransaction(pool, tokenAddress, state) {
  ...
  const ticker = async () => {
    try {
      ...
      const url = `${makerConfig[fromChain].httpEndPoint}/txs?types=Transfer&address=${makerAddress}&token=${tokenInfo ? tokenInfo.id : 0}&start=${startPoint}&limit=50`
      try {
        ...
        if (
          ...
          while (whileTag) {
            if (
              ...
              const moreUrl = `${makerConfig[fromChain].httpEndPoint}/txs?types=Transfer&address=${makerAddress}&token=${tokenInfo ? tokenInfo.id : 0}&start=${startPoint}&limit=50`
  ...
  const ticker = async () => {
    ...
    if (!makerNode) {
      ...
      try {
        ...
        accessLogger.info(`confirmFromTransaction fromChain ${fromChain} add success`)
      } catch (error) {
        errorLogger.error('newTransactionSqlError =', error)
      }
    }
    if (trxConfirmations.confirmations >= confirmations) {
      ...
      try {
        ...
        accessLogger.info(`confirmFromTransaction fromChain ${fromChain} update success`)
      } catch (error) {
        errorLogger.error('updateFromError =', error)
        return
      }
  ...
  await send(
    ...
    if (!response.code) {
      ...
      try {
        ...
        accessLogger.info(`sendTransaction toChain ${toChain} state = 20  update success`)
